# Teddy GitHub协作与每日同步规则

> 更新时间：2026-06-09  
> 用途：记录 Teddy 项目公开 GitHub 仓库、协作方式和当前本地保存规则。

---

## 一、公开仓库

GitHub 仓库：

`https://github.com/l596955124-hue/Teddy`

仓库属性：

- Visibility：Public
- 默认分支：`main`
- 本地目录：`D:\Teddy创业项目`
- 说明：本地总目录已迁移到 D盘一级目录 `D:\Teddy创业项目`，GitHub远程仓库名称暂仍为 `Teddy`。

同事可以通过以下方式克隆：

```bash
git clone https://github.com/l596955124-hue/Teddy.git
```

使用自己的 Codex 工具协作时，建议先读取：

1. `README.md`
2. `项目总控上下文-精简版.md`
3. `跨工作流同步规则.md`
4. `工作流专区/README.md`
5. `项目协作规则.md`

---

## 二、GitHub 自动同步状态

从 2026-06-09 起，Teddy 项目暂停自动同步 GitHub。

原 Codex 自动化任务已删除：

| 项目 | 内容 |
|------|------|
| 自动化ID | `teddy-github` |
| 名称 | Teddy 每日 GitHub 公开同步 |
| 当前状态 | 已删除，不再运行 |
| 删除原因 | 用户要求后续不再自动同步 GitHub |

当前规则：

1. 不自动 `git push`。
2. 不自动把新增内容上传 GitHub。
3. 如确实需要推送 GitHub，必须由刘梓寒明确单独确认。
4. 日常只做本地保存和本地阶段存档。

---

## 三、主线程每日提醒

已创建 Codex 主线程提醒自动化：

| 项目 | 内容 |
|------|------|
| 自动化ID | `teddy` |
| 名称 | Teddy 主线程每日本地保存提醒 |
| 频率 | 每天 |
| 时间 | 北京时间 23:00 |
| 目标 | 当前 Teddy 商业计划书主线程 |

提醒内容包括：

1. 当天是否有跨工作流更新。
2. 是否有需要本地保存的阶段成果。
3. 是否有需要用户确认的风险或事项。
4. 是否有不用的文件建议清理。
5. 是否建议压缩上下文以减少 token 消耗、提升回复速度。
6. 是否有更高效的操作建议。
7. 不执行 GitHub 推送。

---

## 四、公开仓库注意事项

由于仓库是公开的，任何人都可能看到内容。

禁止提交：

- API Key
- Token
- 密码
- `.env` 文件
- Firecrawl缓存
- `node_modules`
- 临时构建目录 `outputs`

可以提交：

- 商业计划书
- PPT
- 品牌资产
- 执行流程
- 公开来源资料
- 跨工作流同步规则

---

## 五、同事协作建议

同事使用自己的 Codex 时，建议这样开场：

```text
请先读取这个仓库中的 README.md、项目总控上下文-精简版.md、跨工作流同步规则.md，然后基于 Teddy 项目的统一口径继续协作。
```

如果同事要修改内容：

1. 先 `git pull`。
2. 修改或新增对应文件。
3. 保存结论到本地文件。
4. 当前不要自行推送 GitHub；如需推送，先让刘梓寒确认。

---

## 六、当前状态

第一次公开推送已完成。

每日 GitHub 自动同步已暂停。

后续只由 `teddy` 在每天 23:00 做本地保存提醒与检查。
